    ...
    <div class="wrapper">
        ...
        <ul class="products-list">
            ...
            <?if(!$arElement['packet']):
                ...
                <li class="product-item <?if($arElement['packet']):?>product-item-set<?endif?>">
                    <?if($arElement['packet']):?>
                    ...
                    <?endif?>
                        <div class="product-item-front">
                            <div class="cart-class"><?if(!$arElement['packet']):?><?=strip_tags($min)?><?if($min!=$max):?> - <?=strip_tags($max)?><?endif?>
                                <?else:?>
                            ...
                            <div class="cart-img-wrap">
                                <?if($arElement['packet']):?>
                                    <img src="<?=$arElement['PREVIEW_PICTURE_SRC']?>" alt="<?=$arElement['NAME']?>" title="<?=$arElement['NAME']?>" class="cart-img">
                                <?else:?>
                                ...
                                <?endif?>
                                <?if($arElement['packet']):?><div class="marker cart-set-number"><?=count($arElement['PROPERTY_BOOKS_VALUE'])?> в 1</div><?endif?>
                            </div>
                        ...
                        <div class="product-item-backside">
                            <div class="product-item-type"><?if(strlen($arElement['PROPERTIES']['SHORT_NAME']['VALUE'])>0) echo $arElement['PROPERTIES']['SHORT_NAME']['VALUE']; else echo TruncateText($arElement['NAME'], 65)?></div>
                            <?if($arElement['packet']):?>
                            ...
                            <?if(!$arElement['packet']):?>
                                <ul class="product-download-list">
                                    ...
                                    <?endif?>
                                    <?if(in_array('Windows',$arElement['PROPERTIES']['PLATFORM']['VALUE'])||count($arElement['PROPERTIES']['PLATFORM']['VALUE'])==0):?>
                            ...
                            <div class="button-block" onmouseover="$(this).parent('div').parent('a').attr('href','javascript:popupCart();');" onmouseout="$(this).parent('div').parent('a').attr('href','/products/<?=$arElement['PROPERTIES']['CID']['VALUE']?>/');">
                                ...
                                <button class="button btn-buy click-btn" onclick="javascript:dataLayer.push({'event':'gaEvent', 'eventCategory':'Cart', 'eventAction':'CartAdd', 'eventLabel':'List', 'eventAdd':{'ContentId': <?if($arElement['packet']):?>'P<?=$arElement["ID"]?>'<?else:?>'<?=$arElement['PROPERTIES']['CID']['VALUE']?>'<?endif?>}});add2basketPacket(<?=$arElement["ID"]?>);" id="button_add_<?=$arElement["ID"]?>" data-page="CIPage">
