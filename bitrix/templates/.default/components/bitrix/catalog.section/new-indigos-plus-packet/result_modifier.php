...
$arResult['SESS']=$_SESSION['LANDING'];
//add packet's data
if(strlen($_SESSION['LANDING']['ID_PACKET'])>0){
    $arMod=array();
    $arFilterPac = Array(
        "IBLOCK_ID"=>11,
        "ID"=>$_SESSION['LANDING']['ID_PACKET'],
        "ACTIVE"=>"Y"
    );
    $resPac = CIBlockElement::GetList(
        Array("SORT"=>"ASC"),
        $arFilterPac,
        false,
        false,
    ...
    $arPacket = $resPac->GetNext();
    $arPacket['PREVIEW_PICTURE_SRC']= CFile::GetPath($arPacket["PREVIEW_PICTURE"]);
    $arPacket['DISPLAY_PROPERTIES']["CLASS"]["DISPLAY_VALUE"][0]= $arPacket["PROPERTY_CLASS_NAME"];
    $arPacket['DISPLAY_PROPERTIES']["SUBJECT"]["DISPLAY_VALUE"]= $arPacket ["PROPERTY_SUBJECT_NAME"];
    $arPacket['packet']=1;
    $arMod=array_chunk($arResult['ITEMS'],2);
    $arResult['ITEMS']=array_merge($arMod[0],array($arPacket),$arMod[1],$arMod[2],$arMod[3],$arMod[4]);
    unset($arResult['ITEMS'][10]);
    unset($_SESSION['LANDING']['ID_PACKET']);
