...
foreach($arResult["ITEMS"] as $cell=>&$arElement)
{
	...
	if(is_array($arElement["PROPERTIES"]["BOOKS"]) && !empty($arElement["PROPERTIES"]["BOOKS"])) //Product has offers
	{
		$res = CIBlockElement::GetList(Array(), array("ID" => $arElement["PROPERTIES"]["BOOKS"]["VALUE"]),false,false,array("ID","NAME","DETAIL_PAGE_URL", "PREVIEW_PICTURE", "CATALOG_GROUP_1"));
		while($ob = $res->GetNextElement())
		{
			$arRes = $ob->GetFields();
			$arRes["PREVIEW_PICTURE"] = CFile::GetFileArray($arRes["PREVIEW_PICTURE"]);
			$arElement["BOOKS"][] = $arRes;
		}
	}
}
