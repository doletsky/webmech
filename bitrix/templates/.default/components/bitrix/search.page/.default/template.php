...
<?
    ...
    <div class="search-result" style="padding-top: 20px;">
        <h1 class="search-title">Результаты поиска: <?=$arResult["REQUEST"]["QUERY"]?></h1>
        <p class="search-text">Найдено <?=$arResult["NAV_RESULT"]->NavRecordCount?> товаров.</p>
    </div>
...
<?foreach($arResult["SEARCH"] as $cell=>$arElement):?>
    ...
    $counter++;
    ?>
    <div class="row <?=($counter == 4) ? 'row-last' : ''?>" id="<?=$this->GetEditAreaId($arElement['ID']);?>">
        <?
        ...
        <?endif?>
        <?php if ($arElement['IBLOCK_ID'] == 11): ?>
            <div class="img">
                <a href="<?=$arElement["DETAIL_PAGE_URL"]?>">
                    <img src="<?php echo CFile::GetPath($arElement['PREVIEW_PICTURE']) ?>" width="130" alt="<?=$arElement["NAME"]?>" title="<?=$arElement["NAME"]?>"/>
                </a>
            </div>
        <?php elseif(intval($arElement['PROPERTIES']['CID']['VALUE']) > 0): ?>
            <div class="img">
                <a href="/products/<?=$arElement['PROPERTIES']['CID']['VALUE']?>/">
                    <img src="<?=(LINK_PREVIEW . $arElement['PROPERTIES']['CID']['VALUE'] . '_logo-s.png')?>" width="130" alt="<?=$arElement["NAME"]?>" title="<?=$arElement["NAME"]?>"/>
                </a>
            </div>
        <?else:?>
            <a href="/products/<?=$arElement['PROPERTIES']['CID']['VALUE']?>/"><div class="no-photo-div-big" style="height:130px; width:130px;"></div></a>
        <?endif?>
        <?
        ...
        ?>
        <div class="titl"><a href="/products/<?=$arElement['PROPERTIES']['CID']['VALUE']?>/" title="<?=$arElement["NAME"]?>">
                <?
                ...
                foreach($arElement["PRICES"] as $code=>$arPrice):?>
                    <?if($arPrice["CAN_ACCESS"] && $arPrice["VALUE"]>0):?>
                        <div class="price">
                            ...
                            <?if($arPrice["DISCOUNT_VALUE"]>0 && $arPrice["DISCOUNT_VALUE"] < $arPrice["VALUE"]):?>
