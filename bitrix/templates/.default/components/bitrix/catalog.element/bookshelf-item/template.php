...
<div class="rows clearfix">
	<div class="img">
		<?if(!empty($arResult['PROPERTIES']['CID']['VALUE'])):?>
			<img src="<?=LINK_PREVIEW . $arResult['PROPERTIES']['CID']['VALUE']?>_logo-s.png" style="max-width:99px; max-height:99px; height:auto; width:auto;" alt="<?=$arResult["NAME"]?>" title="<?=$arResult["NAME"]?>"/>
		<?endif?>
		<?/*print_r($arResult)*/?>
	</div>
	<div class="b-1">
	
		<?if ( isset ( $arResult['DISPLAY_PROPERTIES']['HIT'] ) && $arResult['DISPLAY_PROPERTIES']['HIT'] == 'Hit' ) :?>
			<div class="hit">ХИТ</div>
		<?endif;?>
		<div class="razdel"><a href="#"><?=$arResult['DISPLAY_PROPERTIES']['CONTENT_TYPE']['DISPLAY_VALUE']?></a></div>
		<div class="title">
			<a href="<?=$arResult["DETAIL_PAGE_URL"]?>" rel="external"><?=$arResult['NAME']?></a>
		</div>
	</div>
	<div class="b-2">
		<p>
	...
	<?else:?>
		...
		<?foreach($arResult["PRICES"] as $code=>$arPrice):?>
			<?if($arPrice["CAN_ACCESS"]):?>
				...
				<?else:?>
					<?
					$_SESSION['OLD_PRICE'] += intval( str_replace("р.", "", $arPrice["PRINT_VALUE"]) );
					?>
					<div class="price"><?=str_replace("р.", "", $arPrice["PRINT_VALUE"])?> р.</div>
				<?endif?>
			<?endif;?>
		<?endforeach;?>
	<?endif?>
