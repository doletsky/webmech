...
{
	$arType = CIBlockElement::GetList(array(), array('ID' => $arResult['PROPERTIES']['ED_TYPE']['VALUE']), false, false, array('ID', 'NAME'))->Fetch();
	$arResult['ED_TYPE'] = $arType['NAME'];
}
$arResult['DETAIL_TEXT'] = str_replace('#', '', $arResult['DETAIL_TEXT']);
$arResult['DETAIL_TEXT'] = str_replace('&40;', '(', $arResult['DETAIL_TEXT']);
$arResult['DETAIL_TEXT'] = str_replace('&41;', ')', $arResult['DETAIL_TEXT']);

$arResult['DETAIL_TEXT'] = htmlspecialcharsbx($arResult['DETAIL_TEXT']);
$arResult['DETAIL_TEXT'] = str_replace('&gt;', '>', $arResult['DETAIL_TEXT']);
$arResult['DETAIL_TEXT'] = str_replace('&lt;', '<', $arResult['DETAIL_TEXT']);
$arResult['DETAIL_TEXT'] = str_replace('&amp;', '&', $arResult['DETAIL_TEXT']);
$arResult['DETAIL_TEXT'] = str_replace('&quot;&quot;', '#', $arResult['DETAIL_TEXT']);
$count = null;
$arResult['DETAIL_TEXT'] = preg_replace('/(#)([^#]*)(#)/i', '«$2»', $arResult['DETAIL_TEXT'], -1, $count);
?>

<?//Title для страницы ЕК
$strTitle='';
$strTitle.=$arResult["NAME"];

$strSub='';
foreach($arResult["DISPLAY_PROPERTIES"]["SUBJECT"]["LINK_ELEMENT_VALUE"] as $sub)
{
 $strSub.=" , ".$sub["NAME"];
}
$strTitle.=$strSub;
...
$strClass=', Класс:';
foreach($arResult["DISPLAY_PROPERTIES"]["CLASS"]["LINK_ELEMENT_VALUE"] as $class)
{
    $strClass.=" , ".str_replace(" класс", "", $class["NAME"]);
}
$strTitle.=$strClass;
