...
class CSaleOrderIndigos extends CSaleOrder
{
	...
	function GetList($arOrder = Array("ID"=>"DESC"), $arFilter = Array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $arOptions = array())
	{
		...
		$arFields = array(
            ...
            "UTM_CAMPAIGN_ID" => array("FIELD" => "TUTM.UTM_CAMPAIGN_ID", "TYPE" => "int", "FROM" => "LEFT JOIN (select a.ID as UTM_CAMPAIGN_ID, l.event3 as ORDER_ID from b_stat_event_list l inner join b_stat_event e ON e.id = l.event_id inner join b_stat_adv a ON l.ADV_ID = a.id where e.EVENT2 = 'order_paid') as TUTM on (TUTM.ORDER_ID = O.ID)")
		);
...
//			"SELECT ".$arSqls["SELECT"]." ".
//				$obUserFieldsSql->GetSelect()." ".
//                ", tutm.UTM_CAMPAIGN_ID ".
//			"FROM b_sale_order O ".
//			"	".$arSqls["FROM"]." ".
//				$obUserFieldsSql->GetJoin("O.ID")." "
//            ." LEFT JOIN (select a.ID as UTM_CAMPAIGN_ID, l.event3 as ORDER_ID
//from b_stat_event_list l
//inner join b_stat_event e ON e.id = l.event_id
//inner join b_stat_adv a ON l.ADV_ID = a.id
//where e.EVENT2 = 'order_paid') as tutm on tutm.ORDER_ID = O.ID ";
        $strSql =
			"SELECT ".$arSqls["SELECT"]." ".
				$obUserFieldsSql->GetSelect()." ".
			"FROM b_sale_order O ".
