...
require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/php_interface/include/sale_order_utmstat/CSaleOrderIndigos.php");
require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/php_interface/include/sale_order_utmstat/lang/".LANG.".php");

// include functions
require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/sale/general/admin_tool.php");

function getUtmCampaign($orderId){
    global $DB;
    $orderId = intval($orderId);
    $sql = "select a.ID, a.REFERER1, a.REFERER2
from b_stat_event_list l
left join b_stat_event e ON e.id = l.event_id
left join b_stat_adv a ON l.ADV_ID = a.id
where l.event3 = {$orderId}";// and e.EVENT2 = 'order_paid'";
    $res = $DB->Query($sql);
    if ($row = $res->Fetch()){
        return "<br/>[<a href='adv_detail.php?lang=&amp;find={$row['ID']}&amp;set_filter=Y'>{$row['ID']}</a>] <a href='adv_detail.php?lang=".LANG."&amp;find={$row['ID']}&amp;set_filter=Y'>{$row['REFERER1']} / {$row['REFERER2']}</a>";
    }
//    var_dump($res); die();
    return "";
}
...
if (intval($filter_utm_campaign) > 0){
    $arFilter['UTM_CAMPAIGN_ID'] = intval($filter_utm_campaign);
}
...
$arHeaders = array(
    ...
    array("id"=>"UTM_CAMPAIGN","content"=>GetMessage("UTM_CAMPAIGN"), "sort"=>""),
);
...
$dbOrderList = CSaleOrderIndigos::GetList(
    $arFilterOrder,
    $arFilterTmp,
    $arGroupByTmp,
    array("nPageSize"=>CAdminResult::GetNavSize($sTableID)),
...
while ($arOrder = $dbOrderList->NavNext(true, "f_"))
{
    ...
    $row->AddField("UTM_CAMPAIGN", getUtmCampaign($arOrder['ID']));
...
$lAdmin->AddAdminContextMenu($arSiteMenu);
...
?>
    ...
    $arFilterFieldsTmp = array(
        ...
        "filter_utm_campaign" => GetMessage("UTM_CAMPAIGN")
    );
    ...
    <tr>
        <td><?php echo GetMessage("UTM_CAMPAIGN") ?>:</td>
        <td>
            <select name="filter_utm_campaign">
                <option value=""><?php echo GetMessage("SALE_F_ALL")?></option>
                <?php $utmCampaignsRes = $DB->Query("select ID, CONCAT('[',ID,'] ', COALESCE(REFERER1, ''), ' / ', COALESCE(REFERER2, '')) as NAME from  b_stat_adv a"); ?>
                <?php while ($utmCampaign = $utmCampaignsRes->Fetch()): ?>
                    <option <?php if ($filter_utm_campaign==$utmCampaign['ID']): ?>selected="selected"<?php endif ?> value="<?php echo $utmCampaign['ID'] ?>"><?php echo $utmCampaign['NAME'] ?></option>
                <?php endwhile ?>
            </select>
        </td>
    </tr>
